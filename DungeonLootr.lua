...
local AUTOEXEC_PATH = "AutoExecute/DungeonLootr-SeoulHub.lua"
...
-- Detect if already installed on startup for AutoExecute
...
createToggle(TabSettings, "AutoExecute", "AutoExecute", "Automatically saves or removes the script in Madium's AutoExecute folder.")
        ...
        if State.AutoExecute then
            ...
            if writefile then
                local success, err = pcall(function()
                    ...
                    if makefolder and isfolder and not isfolder("AutoExecute") then
                ...
                if success then
                    SendHubNotification("AutoExecute Enabled & File Saved!", 3)
                else
            ...
            if isfile and isfile(AUTOEXEC_PATH) and delfile then
                ...
                SendHubNotification("AutoExecute Disabled & File Removed!", 3)
            end
        end
    end
end))
